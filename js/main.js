   $(document).ready(function(){
       //메인 메뉴에 마우스 오버했을때 서브배경 슬라이드 다운
       //hover 이벤트는 유일하게 마우스 오버,마우스 아웃 이벤트를 다 가지고있음.
       ...
       //메인 비주얼 슬라이드
       //num 의 li a 에 클릭 이벤트 설정(01,02 버튼)
       $('.num ul li').click(function(){
           //클릭한 li의 인덱스번호를 li_numIndex변수에 저장
       ...
       //.btn a에 클릭이벤트 설정, .btn a 를 클릭하면 자동슬라이드가 멈추고, 다시 클릭하면 자동슬라이드 실행,버튼 모양도 바뀜
       var sw=0;  //스위치변수(버튼 1개 클릭할 떄마다 상태가 변할 때 사용함)
       ...
       //fn함수 선언
       function fn(){
           //.num ul li의 인덱스 번호를 1씩 증가시킴
        numLi += 1;
        //만약 인덱스번호가 2보다 커지면 0으로 초기화
        if(numLi > 2) { numLi=0; }
        // numLi인덱스 번호에 해당하는 li클릭
